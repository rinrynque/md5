        ...
        else if ( param == "mesh" )
        {
            ...
            while ( param != "}" )  // Read until we get to the '}' character
            {
                if ( param == "shader" )
                {
                    ...
                    mesh.m_nmTexID = getNormalMap(mesh.m_hmTexID);
                    //mesh.m_nmTexID = SOIL_load_OGL_texture( nmTexPath.c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, 0 );
                    catchError("chargement textures");
...
bool MD5Model::PrepareMesh( Mesh& mesh, const std::vector<glm::mat4x4>& skel )
{
    for ( unsigned int i = 0; i < mesh.m_Verts.size(); ++i )
    {
        ...
        glm::vec3& bitangent = mesh.m_BitangentBuffer[i];
        ...
        for ( int j = 0; j < vert.m_WeightCount; ++j )
        {
            ...
            bitangent += glm::vec3( ( boneMatrix * glm::vec4( vert.m_Bitangent, 0.0f ) ) * weight.m_Bias );
        }
    }
    return true;
}
...
void MD5Model::RenderGPU( const Mesh& mesh )
{
    ...
    glDisableVertexAttribArray( aID_tangent );
    glDisableVertexAttribArray( aID_bitangent );
